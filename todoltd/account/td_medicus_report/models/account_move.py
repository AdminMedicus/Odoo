...
class AccountMove(models.Model):
    ...
    def td_get_invoice_report_data(self):
        """
            ...
            if move_line_ids:
                ...
                for lot_info in lots.values():
                    ...
                    line_data = {
                        ...
                        'product_name': line.product_id.description_sale,
                        'product_code': line.product_id.default_code or '',
                        'product_serial_numbers': [lot.name] if lot else [],
                        'product_catalog_number': line.product_id.default_code or '',
                        'product_manufacturer': line.product_id.td_manufacturer_directory_res_id.name or '',
                        'storage_conditions': location.mapped('td_condition_ids.name'),
                        'quantity': lot_info['quantity'],
                        'uom': line.product_uom_id.name,
                        'expiration_dates': [lot_info['expiration_date'].strftime('%d.%m.%Y')] if lot_info['expiration_date'] else [],
                        'price_untaxed': line.td_untaxed_price_unit,
                        'price_subtotal': line.td_untaxed_price_unit * lot_info['quantity'],
                    }
                    data['lines'].append(line_data)
            else:
                line_num += 1
